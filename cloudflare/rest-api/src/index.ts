...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (wateringsMatch) {
      ...
      if (method === 'GET') {
        ...
        const { results } = await env.DB
          .prepare('SELECT * FROM watering_events WHERE plant_id = ? ORDER BY watered_at DESC LIMIT ?')
          .bind(id, limit)
          .all()
        return json(results)
      }
      ...
      if (method === 'POST') {
        ...
        await env.DB.prepare(`
          INSERT INTO watering_events (plant_id, volume_ml, notes, watered_at)
          VALUES (?, ?, ?, datetime('now'))
    ...
    if (photosMatch) {
      ...
      if (method === 'GET') {
        const { results } = await env.DB
          .prepare('SELECT * FROM plant_photos WHERE plant_id = ? ORDER BY taken_at DESC')
          .bind(id)
          .all()
        return json(results)
      }
      ...
      if (method === 'POST') {
        ...
        await env.DB.prepare(`
          INSERT INTO plant_photos (plant_id, r2_key, caption, taken_at)
          VALUES (?, ?, ?, datetime('now'))
        `).bind(id, r2Key, caption).run()
    ...
    // GET /weather/daily  GET /weather/latest
